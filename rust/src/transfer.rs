...
pub fn cmd_import(file: &Path, name: Option<&str>, do_switch: bool) -> Result<(), String> {
    ...
    // and the EFI/Pop_OS-<uuid> directory target-correct. Seeding from the
    // real ESP requires the source to be the currently-active overlay.
    ...
    let seed_source = btrfs::get_active_subvol(&cfg).ok().flatten().unwrap_or_else(|| dst.clone());
    crate::commands::customize_overlay_esp(&dst, &format!("@overlay-{final_name}"), &cfg, &seed_source);
